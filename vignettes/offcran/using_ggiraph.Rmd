...
[Read more about custom animation effects](#custom-animation-effects)...
...
> Use function `girafe_options`

With `ggiraph`, you can customize tooltip style, mouse hover effects, toolbar position... 
This requires usage of `css` instructions and options. The function `girafe_options()` 
will be used to customize animation effects. These effects will be defined by function 
calls, i.e. `opts_tooltip()`, `opts_toolbar()`, ...

### Tooltip options

Tooltip can be customized with function `opts_tooltip()` (that will be used as 
a parameter in `girafe_options()` call).
...
```{r}
girafe_options(x, opts_toolbar(saveaspng = FALSE) )
```

### Size options

The resizing behaviour can be interrupted with `opts_sizing(rescale = FALSE)`.

```{r}
girafe_options(x, opts_sizing(rescale = FALSE) )
```
