...
We need to use `label_interactive()` to make 
interactive strip labels and these need to be processed 
by a call to `labeller_interactive()`.

First let's create the function that will create the 
...
lab_custom <- function(z, prefix = "label: ") {
  label_interactive(
    label = z, 
    data_id = z, 
    tooltip = paste0(prefix, z)
    )
}
```

Then let's create the theme object.
