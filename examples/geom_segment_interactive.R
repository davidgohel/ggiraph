...
gg_segment_1 <- ggplot(data = counts, aes(x = x, y = Freq, yend = 0, xend = x, tooltips = xlab ) ) + 
...
dataset = data.frame(x=c(1,2,5,6,8), 
		...
		labs = paste0("Lab", 1:5))
gg_segment_2 = ggplot() + 
		geom_segment_interactive(data=dataset, mapping=aes(x=x, y=y, xend=x+vx, yend=y+vy, tooltips = labs), 
